...
@section('content')
                                ...
                                <div class="noteItemContent">
                                    ...
                                    <div class="file" data-file-id="{{$file->id}}">
                                        ...
                                        <div class="noteItemText">
                                            <a href="{{ asset('/uploads/user_files/'.$file->file) }}" target="_blank">{{$file->name}}</a>
                                            <p>{{$file->description}}</p>
                                        </div>
                                        <div class="noteItemDate">{{$file->created_at->diffForHumans()}}<span class="separator">·</span>{{$file->user()->first()->name}} {{$file->user()->first()->surname}}<span class="separator">·</span>{{parseFilesize($file->size)}} @if($file->active)<span class="separator">·</span><i class="fe-user-check"></i>@endif</div>
                                        <div class="noteItemButtons">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end">
                                                @if($file->active)
                                                    <li><a class="dropdown-item dropdown-item-status">Ukryj</a></li>
                                                @else
                                                    <li><a class="dropdown-item dropdown-item-status">Udostępnij</a></li>
                                                @endif
    ...
    @push('scripts')
        ...
        <script type="text/javascript">
            ...
            filesList.on('click', '.dropdown-item-status', function(event){
                const target = event.target;
                const parent = target.closest(".file");
                $.confirm({
                    title: "Potwierdzenie",
                    message: "Czy na pewno chcesz zmienić widoczność tego pliku?",
                    buttons: {
                        Tak: {
                            "class": "btn btn-primary",
                            action: function() {
                                ...
                                $.ajax({
                                    url: route('admin.crm.clients.file.change-status', [{{ $client->id }}, parent.dataset.fileId]),
                                    type: "POST",
                                    data: {
                                        _token: '{{ csrf_token() }}'
                                    },
