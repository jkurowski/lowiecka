        ...
        <div id="board" class="card mt-3 h-auto">
            <div class="container-fluid">
                ...
                <div class="stages-container issue-stage-container">
                    <div id="stages" class="stages-row h-auto">
                        @php
                            $stages = [
                                1 => 'Nowe',
                                2 => 'W toku',
                                            ...
                                            <div class="task-body">
                                                ...
                                                <div class="container p-0">
                                                    <div class="row issue-stage-desc no-gutters">
                                                        <div class="col-6">Klient:</div><div class="col-6"><a href="{{ route('admin.crm.clients.show', $issue->client) }}">{{ $issue->client->name }} {{ $issue->client->lastname }}</a></div>
                                                        <div class="col-6">Dział:</div><div class="col-6">{{ $issue->department->name }}</div>
                                                        @if($issue->investment_id)
                            ...
                            <div class="card-header">
                                <div class="row">
                                    <div class="col-6 d-flex align-items-center">
                                        <div class="portlet-title">Opis zgłoszenia</div>
                                    </div>
                                </div>
                            </div>
                            <div class="card-body">
                                        ...
                                        @foreach (collect(json_decode($issue->history, true))->sortByDesc('datetime') as $change)
                                            ...
                                            <div class="note">
                                                ...
                                                <div class="noteItemContent">
                                                    <p>Zmiana statusu z <b>{{ issueStatus($change['old_status']) }}</b> na <b>{{ issueStatus($change['new_status']) }}</b> <br><span class="small-text">{{ \Carbon\Carbon::parse($change['datetime'])->format('Y-m-d H:i:s') }} / {{ $user ? $user->name.' '.$user->surname : 'Unknown User' }}</span></p>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
