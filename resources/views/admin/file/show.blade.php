        ...
        <div class="card mt-3">
            <div class="table-overflow">
                ...
                <table id="sortable" class="table mb-0">
                    ...
                    <tr>
                        <td class="text-center"><i class="fe-corner-left-up"></i></td>
                        <td colspan="6">
                            <a href="{{ route('admin.file.index') }}" title="Pliki">[ ... ]</a>
                        </td>
                    </tr>
                    @else
                        <tr>
                            <td class="text-center"><i class="fe-corner-left-up"></i></td>
                            <td colspan="6">
                                <a href="{{ route('admin.file-catalog.show', $entry->ancestors->first()->id) }}" title="{{ $entry->ancestors->first()->name }}">[ ... ]</a>
                            </td>
                        </tr>
                    @endif
                    @foreach ($list as $item)
                        <tr id="recordsArray_{{ $item->id }}">
                            ...
                            <td class="option-120">
                                <div class="btn-group">
                                    @if($item->type == 0)
                                    <button class="btn action-button me-1" onclick="utils.copyToClipboard('{{ asset('uploads/storage/' . $item->file) }}')" data-toggle="tooltip" data-placement="top" title="Skopiuj link do pobierania" target="_blank"><i class="fe-copy"></i></button>
