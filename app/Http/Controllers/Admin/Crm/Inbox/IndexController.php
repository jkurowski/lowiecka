...
class IndexController extends Controller
{
    ...
    {
        $query = ClientMessage::orderByDesc('id')
            ->whereUserId(0)
            ->with([
                'client:id,name,mail,lastname,phone'
            ])
            ->whereDoesntHave('client.properties');
        ...
        if ($request->filled('name') || $request->filled('lastname') || $request->filled('phone') || $request->filled('email')) {
            $query->whereHas('client', function ($q) use ($request) {
                if ($request->filled('name')) {
                    $q->where('name', 'like', '%' . $request->input('name') . '%');
                }
                if ($request->filled('lastname')) {
                    $q->where('lastname', 'like', '%' . $request->input('lastname') . '%');
                }
                if ($request->filled('phone')) {
                    $q->where('phone', 'like', '%' . $request->input('phone') . '%');
                }
                if ($request->filled('email')) {
                    $q->where('mail', 'like', '%' . $request->input('email') . '%');
                }
            });
        }
        ...
        return Datatables::of($list)
            ...
            ->editColumn('lastname', function ($row) {
                return '<a href="' . route('admin.crm.clients.chat.show', $row->client->id) . '">' . $row->client->lastname . '</a>';
            })
            ...
            ->rawColumns(['name', 'lastname', 'created_at', 'actions'])
            ->make();
    }
