...
class GeneratorController extends Controller
{
    ...
    public function __construct(EmailGeneratorService $service, EmailTemplateRepository $repository, InvestmentRepository $investmentRepository)
    {
     ...
     */
    public function index(string $investment_id)
    {
        $list = $this->repository->all();
        $list = $list->where('investment_id', $investment_id);
        return view('admin.email.generator.index', ['list' => $list, 'investment_id' => $investment_id]);
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create(string $investment_id)
    {
        $types = TemplateTypes::getTypes();

        if (!$investment_id == 0 && !$investment_id == null) {
            unset($types[TemplateTypes::NEWSLETTER]);
        }

        return view('admin.email.form', [
            'investment_id' => $investment_id,
            'cardTitle' => 'Stwórz szablon',
            'backButton' => route('admin.email.generator.index', ['investment_id' => $investment_id]),
            'templateTypes' => $types,
            'selectedTemplate' => 0,
            'entry' => new EmailTemplate(['investment_id' => $investment_id])
        ]);
    }
    ...
    {   
        ...
        $template = $this->repository->create($validatedData);
        

        return redirect(route('admin.email.generator.show', ['investment_id' => $validatedData['investment_id'], 'id' => $template->id]));
    }
    ...
    public function show(string $investment_id, int $id)
    {
        $generator = $this->repository->find($id);
        ...
        return view('admin.email.generator.form', [
            ...
            'backButton' => route('admin.email.generator.index', ['investment_id' => $generator->investment_id]),
    ...
    public function edit(string $investment_id, int $id)
    {
        ...
        return view('admin.email.form', [
            ...
            'backButton' => route('admin.email.generator.index', ['investment_id' => $template->investment_id]),
            'templateTypes' => $types,
    ...
    public function update(EmailTemplateFormRequest $request, string $id)
    {
        ...
        return redirect(route('admin.email.generator.index', ['investment_id' => $template->investment_id]))
            ->with('success', 'Szablon zaktualizowany');
    }
    ...
    public function copy(EmailTemplate $email)
    {
        ...
        return redirect()
            ->route('admin.email.generator.index', ['investment_id' => $email->investment_id])
            ->with('success', 'Szablon skopiowany');
    }
